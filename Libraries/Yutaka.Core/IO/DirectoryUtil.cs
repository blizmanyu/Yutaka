		...
		public static IList<string> GetFiles(string path, string searchPattern = "*")
		{
			...
			try {
				...
				if (ex.InnerException == null)
					throw new Exception(String.Format("{0}{2}Exception thrown in DirectoryUtil.EnumerateFiles(string path='{3}', string searchPattern='{4}').{2}{1}{2}{2}", ex.Message, ex.ToString(), Environment.NewLine, path, searchPattern));
				else
					throw new Exception(String.Format("{0}{2}Exception thrown in INNER EXCEPTION of DirectoryUtil.EnumerateFiles(string path='{3}', string searchPattern='{4}').{2}{1}{2}{2}", ex.InnerException.Message, ex.InnerException.ToString(), Environment.NewLine, path, searchPattern));
			}
		}
	}
}