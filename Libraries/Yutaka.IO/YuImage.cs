		...
		private void SetNewFolderAndFilename()
		{
			...
			for (int i = 0; i < Top1000GirlNamesMoreThan4Chars.Length; i++) {
				if (fullnameUpper.Contains(Top1000GirlNamesMoreThan4Chars[i].ToUpper())) {
					NewFolder = String.Format(@"z\{0}\", Top1000GirlNamesMoreThan4Chars[i]);
					NewFilename = Name;
					return; // only match one, then return //
				}
			}
