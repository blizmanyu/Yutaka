...
namespace Yutaka.Google.Gmail
{
	...
	/// 
	/// Available scopes are here: https://developers.google.com/gmail/api/auth/scopes
	...
	public class GoogleGmailService
	{
		...
		public static readonly string DefaultApplicationName = "Yutaka's Gmail Service";
		...
		#region Constructor
		/// <summary>
		/// Creates a new <see cref="GoogleGmailService"/>.
		/// </summary>
		...
		public GoogleGmailService(string applicationName = null, string certificateFileName = null, string certificatePassword = null, string serviceAccountEmail = null, X509KeyStorageFlags certificateKeyStorageFlags = X509KeyStorageFlags.Exportable)
		{
		...
		#region Methods
		/// <summary>
		/// Creates a new Google <see cref="GmailService"/>.
		/// </summary>
		...
		/// <summary>
		/// Creates a new Google <see cref="GmailService"/>. A return value indicates whether the creation succeeded. Any error messages are
		...
		public bool TryCreateService(out string response)
		{
			...
			if (!String.IsNullOrWhiteSpace(response)) {
				response = String.Format("{0}Exception thrown in GoogleGmailService.TryCreateService(out string response).{1}", response, Environment.NewLine);
				return false;
			}
			...
			try {
				...
				if (ex.InnerException == null)
					response = String.Format("{0}{2}Exception thrown in GoogleGmailService.TryCreateService(out string response).{2}{1}{2}{2}", ex.Message, ex.ToString(), Environment.NewLine);
				else
					response = String.Format("{0}{2}Exception thrown in INNER EXCEPTION of GoogleGmailService.TryCreateService(out string response).{2}{1}{2}{2}", ex.InnerException.Message, ex.InnerException.ToString(), Environment.NewLine);
				#endregion Log
